...
class CustomUser(AbstractUser):
    ...
    uid = models.CharField(
        max_length=10,
        default="000000000",
        verbose_name="Roll Number/Employee Number",
...
class MechHomePage(Page):
    ...
    research_profile_documentary_code = models.CharField(
        blank=True,
        null=True,
        max_length=264,
        verbose_name="Research profile documentary video code",
    )
...
class NewsAnnouncementPage(Page):
    ...
    info_type = models.CharField(default="0", choices=TEXT_PANEL_CONTENT_TYPES, max_length=50)
...
class FacultyResearchInterestTag(TaggedItemBase):
    content_object = ParentalKey("FacultyPage", related_name="tagged_items", on_delete=models.CASCADE)
...
class FacultyPage(Page):
    ...
    body = RichTextField(blank=True, features=CUSTOM_RICHTEXT)
    research_interests = ClusterTaggableManager(
        through=FacultyResearchInterestTag,
        blank=True,
    ...
    on_insti = models.CharField(
        max_length=500,
        blank=True,
        verbose_name="Visiting from/On lien to/On deputation to",
...
class AbstractStudentPage(Page):
    ...
    specialization = models.CharField(
        max_length=2,
        choices=MASTERS_SPECIALIZATION,
        default="0",
...
class StudentHomePage(AbstractStudentHomePage):
    ...
    def serve(self, request):
        ...
        student_list = student_list.order_by(
            "-studentpage__enrolment_year__year",
            "studentpage__first_name",
            "studentpage__middle_name",
            "studentpage__last_name",
        )
...
class StaffHomePage(Page):
    ...
    def serve(self, request):
        staff_list = (
            ...
            .live()
            .order_by(
                "staffpage__first_name",
                "staffpage__middle_name",
...
class StaffSkillag(TaggedItemBase):
    content_object = ParentalKey("StaffPage", related_name="tagged_items", on_delete=models.CASCADE)
...
class ProjectStaff(Page):
    ...
    associate_faculty = models.ForeignKey(
        "FacultyPage",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
        related_name="associate_faculty",
    )
...
class AlumniInterestTag(TaggedItemBase):
    content_object = ParentalKey("AlumnusPage", related_name="alumni_tagged_items", on_delete=models.CASCADE)
...
class DistinguishedAlumni(Orderable):
    ...
    distinguished_alumnus = models.ForeignKey(
        "AlumnusPage",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
...
class FeaturedResearch(Orderable):
    ...
    featured_research = models.ForeignKey(
        "PublicationPage",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
...
class ResearchPageGalleryImage(Orderable):
    page = ParentalKey(
        ResearchLabHomePage,
        on_delete=models.CASCADE,
        related_name="lab_home_gallery_images",
    )
    image = models.ForeignKey("wagtailimages.Image", on_delete=models.CASCADE, related_name="+")
    caption = models.CharField(blank=True, max_length=250)
    panels = [
...
class ResearchLabPage(Page):
    ...
    scientific_officer = models.ForeignKey(
        "StaffPage",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
...
class Program(Page):
    ...
    def hasSpecialization(self):
        return self.get_children().type(ProgramSpecialization).exists()
    ...
    def get_context(self, request):
        ...
        context["programSpecializations"] = self.get_specializations()
    ...
    def get_specializations(self):
        return self.get_children().type(ProgramSpecialization)

    def get_siblings_with_alumni_details(self):
        siblings = Program.objects.sibling_of(self).filter(has_alumni_details=True)
        return siblings
...
class ProgramSpecialization(Page):
    ...
    def get_siblings_with_alumni_details(self):
        siblings = ProgramSpecialization.objects.sibling_of(self).filter(has_alumni_details=True)
        return siblings
...
class ProgramTestimonials(Orderable):
    ...
    panels = [FieldPanel("testimonial"), FieldPanel("author")]
        ...
        def visitNode(node, html):
            for child in node.get_children():
                ...
                if child.specific_class == EffectiveTimePeriod:
                    html += "course structure (" + child.title + ")"
                else:
                    html += child.title
                html += "</span>" + "</i></li></a>"
                if flag:
